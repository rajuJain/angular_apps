...
angular.module('AngularDemo')
    ...
    */
    ...
    .controller('financialGoalsController', ['$scope','$rootScope', '$validator', 'serviceData','serviceAuth','$location', function ($scope, $rootScope, $validator, serviceData, serviceAuth, $location){
        $rootScope.popupData = null;
        $scope.results = [];
        $rootScope.modalOpen();
        serviceData.get('getSettings',{key:"goal_popup"}).then(function (respo) {
            console.log(respo);
            $rootScope.popupData = respo.result.GOAL_VALUE;
        },function(error){
            $scope.errorMsg = "Connected but received a server error in response: " + error.status;
        });    
